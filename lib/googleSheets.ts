...
// URL MAESTRA (Asegúrate de que en constants.tsx sea la misma proporcionada: ...Jlpw/exec)
...
export async function fetchTableData(tabName: string, ignoreCache = false): Promise<any[]> {
  ...
  const fetchPromise = (async () => {
    try {
      // Usamos GET para lectura (evita problemas de preflight CORS)
      ...
      if (json.error) {
        console.warn(`Script reportó error en ${tabName}:`, json.error);
        return [];
      }
      ...
      console.error(`Error de red fetching ${tabName}:`, err);
...
async function sendToScript(payload: any) {
  try {
    // IMPORTANTE: 'text/plain' evita el preflight OPTIONS de CORS que suele causar 'Failed to fetch'
    ...
    console.error("Error crítico de comunicación POST:", err);
...
export async function updateShareholderPin(uid: string, newPin: string): Promise<{success: boolean}> {
  ...
  return { success: res.success || res.status === 'success' };
}
...
function normalizeSections(rawJson: string): ReportSection[] {
  ...
  try {
    ...
    return [{ titulo: 'Contenido', parrafos: [rawJson], items: [] }];
  }
}
...
export async function updateTableData(tabName: string, data: any): Promise<{success: boolean, message: string}> {
  const res = await sendToScript({ tab: tabName, action: 'update', data: data });
  return { success: res.success, message: res.success ? "OK" : "Error de red" };
}
...
export const fetchExecutionsFromApi = async (category: MarketCategory = 'forex'): Promise<ExecutionData> => {
  try {
    const url = `${PROFILE_API_URL}?tab=${category.toUpperCase()}_OPEN&t=${Date.now()}`;
    ...
    const mapRecord = (item: any, isTradeOpen: boolean): Execution => ({
      ...
      symbol: String(findValue(item, ['symbol', 'simbolo', 'asset']) || '---'),
      side: String(findValue(item, ['accion', 'side', 'type']) || '---'),
      lots: String(findValue(item, ['lotes', 'lots', 'volume']) || '0.00'),
      open_time: String(findValue(item, ['fechadeapertura', 'opentime']) || ''),
      close_time: isTradeOpen ? 'PENDIENTE' : String(findValue(item, ['fechadecierre', 'closetime']) || ''),
      open_price: String(findValue(item, ['preciodeapertura', 'open_price']) || '0.00'),
      close_price: isTradeOpen ? '---' : String(findValue(item, ['preciodecierre', 'close_price']) || '0.00'),
      sl: String(findValue(item, ['slprecio', 'sl']) || '0.00'),
      tp: String(findValue(item, ['tpprecio', 'tp']) || '0.00'),
      profit: parseSheetNumber(findValue(item, ['beneficioneto', 'profit'])),
      gain: String(findValue(item, ['ganancia', 'gain']) || '0.00'),
      swap: '0.00',
      commission: '0.00',
      comment: '',
      isOpen: isTradeOpen
    ...
    return {
      closed: rows.filter((r: any) => findValue(r, ['STATUS', 'status']) === 'CLOSED').map((r: any) => mapRecord(r, false)),
      open: rows.filter((r: any) => findValue(r, ['STATUS', 'status']) !== 'CLOSED').map((r: any) => mapRecord(r, true))
    };
  } catch (error) {
    return { closed: [], open: [] };
  }
